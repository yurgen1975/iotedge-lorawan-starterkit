    ...
    public class JoinRequestMessageHandler
    {
        ...
        async Task ProcessJoinRequestAsync(LoRaRequest request)
        {
            ...
            try
            {
                ...
                if (request.LoRaRegion.RegionLimits.IsCurrentDRIndexWithinAcceptableValue(loRaDevice.RX2DataRate))
                {
                    dlSettings[0] =
                        (byte)(loRaDevice.RX2DataRate & 0b00001111);
                }
                else
                {
                ...
                if (loRaDevice.RX1DROffset >= 0 && loRaDevice.RX1DROffset < request.LoRaRegion.RX1DROffsetTable.GetUpperBound(1))
                {
                    var rx1droffset = (byte)(loRaDevice.RX1DROffset << 4);
                    if (dlSettings == null)
                    {
